import board
import digitalio
import requests
from bs4 import BeautifulSoup
import threading
import json
from PIL import Image, ImageDraw, ImageFont
import adafruit_ssd1306
import time

sisae = ""
url = "https://crix-api-endpoint.upbit.com/v1/crix/candles/days/?code=CRIX.UPBIT.KRW-BTC"

def getBit():
    global sisae
    while True:
        response = requests.get(url)
        html = response.text
        ht = html[1:len(html)-1]
        result = json.loads(ht)
        sisae = str(format(int(result["tradePrice"]), ',')) + "W"
        time.sleep(0.5)

t = threading.Thread(target=getBit)
t.start()


# Setting some variables for our reset pin etc.
RESET_PIN = digitalio.DigitalInOut(board.D4)

# Very important... This lets py-gaugette 'know' what pins to use in order to reset the display
i2c = board.I2C()
oled = adafruit_ssd1306.SSD1306_I2C(128, 64, i2c, addr=0x3c, reset=RESET_PIN)

# Clear display.
oled.fill(0)
oled.show()

# Create blank image for drawing.
image = Image.new("1", (oled.width, oled.height))
draw = ImageDraw.Draw(image)

# Load a font in 2 different sizes.
font = ImageFont.truetype("/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf", 28)
font2 = ImageFont.truetype("/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf", 18)

# Draw the text

while True:
    image = Image.new("1", (oled.width, oled.height))
    draw = ImageDraw.Draw(image)
    draw.text((0, 0), "BITCOIN", font=font, fill=255)
    draw.text((0, 46), sisae, font=font2, fill=255)
    time.sleep(0.2)
   
    # draw.text((34, 46), "Um....", font=font2, fill=255)
    oled.image(image)
    oled.show()




draw.text((0, 30), sisae, font=font2, fill=255)
draw.text((34, 46), "Um....", font=font2, fill=255)

# Display image
oled.image(image)
oled.show()